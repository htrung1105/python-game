...
class Game:
  ...
  def __init__(self, diff, dim, path, start_x=0, start_y=0, end_x=1, end_y=1):
    self.size = (800,600)
    ...
    self.cx, self.cy = start_x, start_y # (x, y) (0-indexed)
    self.end_x, self.end_y = 1 + end_x, 1 + end_y # (end_x, end_y) is (x, y) (0-indexed)
  ...
  def reset_player(self):
    ...
    self.screen.blit(self.goldy, ((self.end_x - 1)*self.cell_width + 2, (self.end_y - 1)*self.cell_height + 2))
  ...
  def move_player(self, dir):
    ...
    # Check for victory.
    if self.cx + 1 == self.end_x and self.cy + 1 == self.end_y:
